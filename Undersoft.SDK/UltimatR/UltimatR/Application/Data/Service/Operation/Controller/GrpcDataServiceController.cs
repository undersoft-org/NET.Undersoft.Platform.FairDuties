...
namespace UltimatR
{
    [LinkedResult]
    [IgnoreApi]
    public abstract class GrpcDataServiceController<TKey, TStore, TEntity> : ODataController, IDsoController<TKey, TStore, TEntity> where TEntity : Entity where TStore : IDataStore
    {
        ...
        protected GrpcDataServiceController(IUltimatr ultimatr, PublishMode publishMode = PublishMode.PropagateCommand) : this(ultimatr, k => e => k.Equals(e.Id), publishMode)
        {
        }
        protected GrpcDataServiceController(IUltimatr ultimatr, Func<TKey, Expression<Func<TEntity, bool>>> keymatcher, PublishMode publishMode = PublishMode.PropagateCommand)
        {
