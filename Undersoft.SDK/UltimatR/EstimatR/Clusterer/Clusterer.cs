...
namespace EstimatR
{
    public class Clusterer
    {
        ...
        public void Create(ICollection<Item> itemCollection)
        {
        ...
        public void Append(ICollection<Item> itemCollection)
        {
        ...
        public void AssignCluster(Item item)
        {
        ...
        public Item SimilarTo(Item item)
        {
            ...
            else
            {
                ...
                List<Item> clusterItemList = cluster.ClusterItemList;
        ...
        public Item SimilarInGroupsTo(Item item)
        {
            StringBuilder outputText = new StringBuilder();
            double tempItemSimilarSum = 0;
            double itemSimilarSum = 0;
            ...
            else
            {
                ...
                List<Item> hyperClusterItemList = hyperCluster.GetHyperClusterItemList();
        ...
        public Item SimilarInOtherGroupsTo(Item item)
        {
            StringBuilder outputText = new StringBuilder();
            double tempItemSimilarSum = 0;
            double itemSimilarSum = 0;
            ...
            else
            {
                ...
                for (int j = 0; j < hyperCluster.ClusterList.Count; j++)
                {
                    ...
                    {
                        List<Item> clusterItemList = hyperCluster.ClusterList[j].ClusterItemList;
                        for (int i = 0; i < clusterItemList.Count; i++)
                        {
                            tempItemSimilarSum = CaulculateVectorIntersectionMagnitude(item.Vector, clusterItemList[i].Vector) / CalculateVectorMagnitude(clusterItemList[i].Vector);    //||item(Reference) and itemFromCluster||/ ||itemFromcluster|| => max ||
                            if (itemSimilarSum == 0 || itemSimilarSum < tempItemSimilarSum)
        ...
        public static double[] CalculateIntersection(List<Item> input, double[] output)
        {
        ...
        public static double[] CalculateSummary(List<Item> input, double[] output)
        {
            for (int i = 0; i < output.Length; i++)
            {
                output[i] = 0;
        ...
        public static double[] UpdateIntersectionByLast(List<Item> input, double[] output)
        {
            int n = input.Count - 1;
            for (int i = 0; i < output.Length; i++)
            {
        ...
        public static double[] UpdateSummaryByLast(List<Item> input, double[] output)
        {
            int n = input.Count - 1;
            for (int i = 0; i < output.Length; i++)
            {
        ...
        public static List<Item> NormalizeItemList(List<Item> featureItemList)
        {
            List<Item> normalizedItemList = new List<Item>();
            ...
            for (int i = 0; i < featureItemList.Count; i++)
            {
                ...
                normalizedItemList.Add(new Item(featureItemList[i].Id, featureItemList[i].Name, featureVector));
            }
            ...
            else
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    ...
                    if (line == "--")
                    {
                        ...
                        ItemList.Add(new Item(featureItemId, featureName, featureVector));
                        featureItemId++;
                    }
                }
            }
