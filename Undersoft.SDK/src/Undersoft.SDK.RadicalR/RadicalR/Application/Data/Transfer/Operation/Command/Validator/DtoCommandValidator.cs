...
namespace RadicalR
{
    public class DtoCommandValidator<TDto> : CommandValidator<DtoCommand<TDto>> where TDto : Dto
    {
        public DtoCommandValidator(IServicer ultimateService) : base(ultimateService)
        {
        }
        ...
        protected void ValidateLength(int min, int max, params Expression<Func<DtoCommand<TDto>, string>>[] members)
        {
            foreach (var member in members)
            {
                RuleFor(member)
        ...
        protected void ValidateCount(int min, int max, params Expression<Func<DtoCommand<TDto>, ICollection>>[] members)
        {
            foreach (var member in members)
            {
                RuleFor(member)
