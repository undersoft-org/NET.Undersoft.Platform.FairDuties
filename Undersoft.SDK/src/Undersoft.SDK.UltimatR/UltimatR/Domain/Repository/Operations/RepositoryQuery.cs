...
namespace UltimatR
{
    public partial class Repository<TEntity>
    {
        ...
        )
        {
            return Task.Run(
                () =>
                    (IList<TResult>)(
                        (take > 0)
                            ? Query.Select(selector).Skip(skip).Take(take).ToAlbum()
        ...
        public virtual Task<IDeck<TEntity>> Filter(
            int skip,
            int take,
            SortExpression<TEntity> sortTerms
        )
        ...
        public virtual Task<IDeck<TEntity>> Filter(IQueryable<TEntity> query)
        {
            return Task.Run(() => query.ToAlbum() as IDeck<TEntity>, Cancellation);
        }

        public virtual Task<IDeck<TEntity>> Get(
            params Expression<Func<TEntity, object>>[] expanders
        )
        {
