...
namespace RadicalR
{
    ...
    {
        private static Catalog<IntPtr> spots;

        static WorkSpot()
        {
            spots = new Catalog<IntPtr>(true, PRIMES_ARRAY.Get(7));
        }

        public static IEnumerable<T> GetRange<T>(long id)
        {
            if (spots.TryGet(id, out ICard<IntPtr> ptrs))
        ...
        {
            if (spots.TryGet(ThreadId, out ICard<IntPtr> ptrs))
                return ptrs.ForEach(p => Target<T>(p));
            return default;
        }
        ...
        {
            if (spots.TryGet(typeof(T).FullName.UniqueKey64((uint)id), out ICard<IntPtr> ptrs))
                return ptrs.ForEach(p => Target<T>(p));
            return default;
        }
        ...
        public static IEnumerable<T> GetTypedRange<T>()
        {
            ...
            if (spots.TryGet(key, out ICard<IntPtr> ptrs))
                return ptrs.ForEach(p => Target<T>(p));
            return default;
        }
