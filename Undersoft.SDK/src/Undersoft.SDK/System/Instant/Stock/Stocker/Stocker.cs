...
namespace System.Instant.Stock
{
    ...
    {
        public Stocker() : this(new Figures<T>().Combine()) { }
    ...
    public class Stocker : IStocker
    {
        ...
        public Stocker(Type type) : this(new Figures(type, FigureMode.Reference).Combine()) { }
        ...
        public void Open()
        {
            ...
            {
                var _options = new SectorOptions(figures, 0, 0);
                IStock sector = new ArrayStock(_options);
                IStock[] cluster = GetCluster(sector.ClusterId);
                cluster[sector.SectorId] = sector;
            }
        ...
        {
            var serialcode = figure.UniqueCode;
            if (!serialcode.GetFlagBit(0))
            {
                var sector = GetSector(serialcode);
                if (sector != null)
        ...
        {
            return GetSector(figure.UniqueCode);
        }
